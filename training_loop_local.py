...
dataset_path = 'empirical_train_dataset_v5_payload_seq.pkl'

X, y, payload_label = data_preproc(chop_reads=1, dataset_path=dataset_path, sample=True)
    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(len(X_train))):
            ...
            if i % 100 == 0:
                ...
                print(f"Payload - {payload_label[i]}")
                ...
                with open(file_write_path, 'a') as f:
                    f.write(f"\nEpoch {epoch} Batch {i} Main Loss {loss.item()} Gt Loss{alpha*sum([i.item() for i in gt_loss_arr])} ")
